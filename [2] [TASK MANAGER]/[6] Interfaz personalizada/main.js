...
// [Clase 6]
const toggleThemeButton = document.getElementById('toggle-btn'); // Obtenemos el boton toggle theme
const mainContainer = document.getElementById('main-container') // Obtenemos el contenedor principal


const currentTheme = localStorage.getItem('theme'); // Obtenemos el tema actual del localStorage
console.log('[CurrentTheme]:', currentTheme); // Imprimimos en consola el tema actual

toggleThemeButton.addEventListener('click', () => { // Agregamos un evento al boton toggle theme cada que le den 'click'
    document.body.classList.toggle('dark-theme'); // Agregamos o quitamos la clase dark-theme al body
    toggleThemeButton.classList.toggle('dark-theme-btn'); // Agregamos o quitamos la clase dark-theme-btn al boton toggle theme
    mainContainer.classList.toggle('dark-theme-container'); // Agregamos o quitamos la clase dark-theme-container al contenedor principal
    
    const allTask = taskList.querySelectorAll('li'); // Obtenemos todos los elementos li de la lista
    allTask.forEach(task =>{ // Recorremos los elementos li
        task.classList.toggle('dark-theme-li'); // Agregamos o quitamos la clase dark-theme-li a cada elemento li
    })
    const theme = document.body.classList.contains('dark-theme') ? 'dark' : 'light'; // Obtenemos el tema actual
    localStorage.setItem("theme", theme); // Guardamos el tema actual en localStorage
});

if (currentTheme === 'dark') { // Si el tema actual es dark
    document.body.classList.add('dark-theme') // Agregamos la clase dark-theme al body
    toggleThemeButton.classList.add('dark-theme-btn') // Agregamos la clase dark-theme-btn al boton toggle theme
    mainContainer.classList.add('dark-theme-container') // Agregamos la clase dark-theme-container al contenedor principal
    const allTasks = taskList.querySelectorAll('li'); // Obtenemos todos los elementos li de la lista
    allTasks.forEach(task => { // Recorremos los elementos li
        task.classList.add('dark-theme-li'); // Agregamos la clase dark-theme-li a cada elemento li
    }
)};